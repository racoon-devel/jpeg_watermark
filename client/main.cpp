...
int main(int argc, char **argv)
{
    ...
    // Parse options
    cxxopts::Options options("Watermark Client", "Client for the demo JPEG Watermark server");

    options.add_options()
        ("a,addr", "Address", cxxopts::value<string>()->default_value("127.0.0.1"), "Server IPv4 address")
        ...
        ("t,timeout", "Timeout", cxxopts::value<uint>()->default_value("10"), "Server reconnect interval")
        ("c,clients", "Clients", cxxopts::value<uint>()->default_value("1"), "Clients count")
        ("o,output", "Output", cxxopts::value<string>()->default_value("output.jpg"), "Image path for result")
        ("help", "Print help")
        ;

    string addr; int port; string text; 
    uint timeout; uint clients_count; 
    ...
    for (const auto& client : clients)
    {
        ...
        {
            save_image(clients_count != 1 ? to_string(idx + 1) + "_" + output_path : output_path, client.image());
        }
