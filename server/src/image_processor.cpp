...
ImageProcessor::ImageProcessor(asio::io_service& io, uint max_jobs)
	: m_thread_count(std::thread::hardware_concurrency()), m_max_jobs(max_jobs),
	  m_io(io), m_stop(false), m_now_running(0)
{
	if (!m_thread_count)
		m_thread_count = m_default_threads;

	// Нет смысла порождать много потоков, если макс. кол-во одновременно
	// работабщих задач ограничено
		...
		m_threads.emplace_back(
			[this, idx]
			{
				...
				while (true)
				{
					...
					m_io.post([session, retval, result]
							  { session->complete(retval, result); });
...
bool ImageProcessor::draw_watermark(const Image& image, const std::string& text,
									Image& result)
{
	try
	{
