...
int Server::Run()
{
	try
	{
		...
		LOG(INFO) << "Server started [ addr = " << m_addr
				  << ", port = " << m_port << ", max jobs = " << m_max_tasks
				  << " ]";

		m_proc.Run();

		/* Таймер нужен для очистки завершенных сессий - не очень красиво, но
		 * здесь большой нагрузки не ожидается */
		m_timer.expires_from_now(std::chrono::milliseconds(m_clear_timeout));
		m_timer.async_wait(
			std::bind(&Server::on_tick, this, std::placeholders::_1));
...
void Server::on_accept(const asio::error_code& ec)
{
	...
	auto session_ptr =
		std::make_shared< ProtoSession >(m_proc, std::move(m_sock));
...
void Server::on_tick(const asio::error_code& ec)
{
	...
	m_sessions.remove_if([](SessionPtr& session)
