...
void Session::receive(uint8_t* buffer, size_t size)
{
	...
	asio::async_read(m_sock, asio::buffer(buffer, size),
					 std::bind(&Session::on_receive_internal, this,
...
void Session::send(const uint8_t* buffer, size_t size)
{
	...
	asio::async_write(m_sock, asio::buffer(buffer, size),
...
void ProtoSession::on_receive()
{
	...
	case State::kReadHeader:
		LOG(DEBUG) << this << "New request { text size: " << m_header.text_size
				   << " image size: " << m_header.image_size << " }";

		if (m_header.text_size > m_max_text_size
			|| m_header.image_size > m_max_image_size)
		{
